...
// 外部引入
#define REBOOT_DEV_OFF          _Module_PWR_LOW                  // 重启模块标记置高
#define REBOOT_DEV_ON           _Module_PWR_HIGH                 // 重启模块标记置低
...
#define CmdStrLenMax LONG_UARTMAX   // 最大字符串长度
...
typedef struct _NetDevParameter {
    ...
    uint8_t CheckOnlineFlag;    // 检查网络在线标记
    uint8_t ReBootCount;        // 重启次数

    char NetDataBuff[CmdStrLenMax + 1]; // 接收数据或发送数据的缓存空间
    ...
    unsigned char isGetDownCmd;         // 是否主动获取指令
} NetDevParameter;
extern NetDevParameter Now_NetDevParameter;
extern bool checkUart(int MaxTimeMs);
// 公共接口
...
#include "../Interflow/_Module_ATCmd_FunctionSum.h"
#endif
